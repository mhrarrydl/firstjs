...
app.post("/gadai-baru", upload.single("photo"), (req, res) => {
  ...
  res.send(renderAutoPrint(record.id));
});
...
app.post("/aktif/:id/tebus", (req, res) => {
  ...
  if (!req.body.confirmed) {
    return res.send(renderLayout("Konfirmasi Tebus", renderConfirmTebus(record, feeSummary)));
  }
  ...
  const body = `
    <div class="ticket">
      <div class="content">
        ...
        <div class="row"><span class="label">Tipe Fee</span><span class="value">${data.feeType === "depan" ? "Depan" : "Belakang"}</span></div>
...
function renderConfirmTebus(record, feeSummary) {
  const totalTebus = record.amount + feeSummary.feeDue;
  return `
    <section class="panel confirm-panel">
      <div class="confirm-card">
        ...
        <div class="confirm-details">
          ...
          <div>
            <p class="label">Total Tebus</p>
            <p class="value">${rupiah(totalTebus)}</p>
          </div>
        </div>
        <div class="confirm-actions">
          <a class="ghost" href="/aktif">Batal</a>
